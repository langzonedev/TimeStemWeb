const VARIANTS={
  ...
  enterprise:{label:'Work',prompt:'What are you working on?',actions:[['start','Start work','stamp'],['project','Project / Job','timer'],['meeting','Meeting','timer'],['support','Support','timer'],['admin','Administration','timer'],['training','Training','timer'],['break','Break','timer'],['finish','Finish work','stamp']]},
...
function renderSummary(main){
  ...
  main.innerHTML=`<section class="summary-hero"><div class="eyebrow">THIS WEEK · ${VARIANTS[variant].label.toUpperCase()}</div><div class="summary-number">${duration(total)}</div><p>${summaryCopy()}</p></section><section class="card summary-list">${rows.length?rows.map(([n,v])=>`<div class="summary-row"><div><strong>${escapeHtml(n)}</strong><div class="summary-track"><span style="width:${total?Math.max(8,(v/total)*100):0}%"></span></div></div><strong>${duration(v)}</strong></div>`).join(''):'<div class="empty-inline"><span class="muted">No timed activity recorded this week.</span></div>'}</section><button class="primary wide" id="copy-summary">Copy weekly summary</button>`;
...
function render(){
  ...
  $('#page-title').textContent='Every tap builds your day.';
